...
export const useCallJoin = () => {
  const { state, dispatch, mediasoup, livekit, session, useLiveKitFlag } = useCallContext();
  
  // Use LiveKit or Mediasoup based on feature flag
  const callClient = useLiveKitFlag ? livekit : mediasoup;
  ...
  const handleJoin = useCallback(async () => {
    if (!state.callId || !callClient.connected) {
      return;
    }
    ...
    try {
      ...
      const joinRes = await callClient.joinRoom(state.callId);
      console.log("[Call] Join response:", joinRes);

      if (useLiveKitFlag) {
        // LiveKit doesn't need RTP capabilities or device loading
        dispatch({ type: "SET_PRODUCERS", payload: [] });
        console.log("[Call] Joined LiveKit room");
      } else {
        // Mediasoup flow
        ...
        await callClient.loadDevice(rtpCapabilities);
        await callClient.createSendTransport();
      }
      if (!useLiveKitFlag) {
        // Mediasoup flow
        await callClient.createRecvTransport();
      ...
      if (useLiveKitFlag) {
        // LiveKit flow
        myProducers = await callClient.createLocalTracks();
        console.log("[Call] LiveKit tracks result:", myProducers);
      } else {
        // Mediasoup flow
        myProducers = await callClient.produce(stream);
        console.log("[Call] Mediasoup production result:", myProducers);
      }
