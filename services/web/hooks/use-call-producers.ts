...
export const useCallProducers = () => {
  const { state, mediasoup, livekit, useLiveKitFlag } = useCallContext();
  ...
  useEffect(() => {
    if (useLiveKitFlag) {
      // LiveKit handles track consumption automatically
      return;
    }
  ...
  useEffect(() => {
    if (useLiveKitFlag) {
      // LiveKit handles new track events automatically
      return;
    }
