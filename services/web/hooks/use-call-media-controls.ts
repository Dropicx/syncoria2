...
export const useCallMediaControls = () => {
  const { state, dispatch, mediasoup, livekit, useLiveKitFlag } = useCallContext();
  ...
  const toggleCamera = useCallback(() => {
    if (useLiveKitFlag) {
      ...
      dispatch({ type: "SET_LOCAL_CAMERA_ON", payload: !state.isLocalCameraOn });
    } else {
      // Mediasoup flow
  ...
  }, [useLiveKitFlag, callClient, mediasoup.localStream, mediasoup.setProducerMuted, state.myProducers, state.isLocalCameraOn, dispatch]);
  ...
  }, [useLiveKitFlag, callClient, mediasoup.localStream, mediasoup.setProducerMuted, state.myProducers, state.isLocalMicOn, dispatch]);

  const handleToggleScreenShare = useCallback(async () => {
    try {
      if (useLiveKitFlag) {
        // LiveKit flow
        await callClient.toggleScreenShare();
        dispatch({ type: "SET_SCREEN_SHARING", payload: !state.isScreenSharing });
      } else {
        // Mediasoup flow
