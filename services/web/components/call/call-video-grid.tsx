...
import { useCallSelector, useMediasoupSelector, useCallContext } from "@/contexts/call-context";
...
const CallVideoGridComponent = memo(() => {
  const { useLiveKitFlag, livekit, mediasoup } = useCallContext();
  ...
  const localStream = useLiveKitFlag ? livekit.localStream : useMediasoupSelector((m) => m.localStream);
  const remoteStreams = useLiveKitFlag ? livekit.remoteStreams : useMediasoupSelector((m) => m.remoteStreams);

  const session = useSession();
  const userImage = session?.user?.imageUrl || "/avatars/default.jpg";
