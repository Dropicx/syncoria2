...
export function CallRoom({ id }: { id: string }) {
  ...
  useEffect(() => {
    // Only handle join requests for Mediasoup (LiveKit handles this differently)
    if (useLiveKitFlag || !mediasoup.socket) return;
  ...
  }, [mediasoup.socket, state.isCreator, state.callId, playNotificationSound, useLiveKitFlag]);

  useEffect(() => {
    // Only handle join approvals for Mediasoup (LiveKit handles this differently)
    if (useLiveKitFlag || !mediasoup.socket) return;
      ...
      {!state.joined ? (
        ...
        <>
          ...
          <ChatSidebar
            ...
            userAvatar={session?.user?.imageUrl || "/avatars/default.jpg"}
